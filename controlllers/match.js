...
export class MatchController {
    ...
    getAll = async (request, reply) => {
        try {
            const results = await this.matchModel.findAll()
    ...
    getById = async (req, res) => {
        try {
            const { id } = req.params
            const player = await this.matchModel.findByPk(id)
    ...
    create = async (req, res) => {
        try {
            ...
            const newPlayer = await this.matchModel.create(result.data)
    ...
    delete = async (req, res) => {
        const { id } = req.params
        const result = await this.matchModel.destroy({ where: { id: id, }, })
        if (result === false) {
    ...
    update = async (req, res) => {
        try {
            ...
            const { id } = req.params
            const updatedPlayer = await this.matchModel.update(result.data, { where: { id: id } })
